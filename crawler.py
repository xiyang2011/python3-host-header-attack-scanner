    ...
    def crawl(self):
        ...
        while not q.empty():
            ...
            if depth == 0 and [] != do_not_follow:
                print("Whoops! Starting URL %s rejected by the following filters:", do_not_follow, file=sys.stderr)
            ...
            if [] == do_not_follow:
                try:
                    ...
                    print("ERROR: Can't process url '%s' (%s)" % (this_url, e), file=sys.stderr)
                    #print format_exc()
    ...
    def fetch(self):
        ...
        if handle:
            try:
                ...
                print("Skipping %s, has type %s" % (error.url, error.mimetype), file=sys.stderr)
                tags = []
            for tag in tags:
                href = tag.get("href")
                if href is not None:
...
def main():    
    ...
    print("Crawling %s (Max Depth: %d)" % (url, depth_limit), file=sys.stderr)
    ...
    print("Found:    %d" % crawler.num_links, file=sys.stderr)
    print("Followed: %d" % crawler.num_followed, file=sys.stderr)
    print("Stats:    (%d/s after %0.2fs)" % (
            int(math.ceil(float(crawler.num_links) / tTime)), tTime), file=sys.stderr)
